...
name: instalar
description: >
  Instala o JN-IA OS no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o JN-IA OS", "primeiro setup".
---

# /instalar — Instalação inicial do JN-IA OS
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `jn-ia-os`, `JN-IA OS`, `JN-IA-OS-main`, `jn-ia-os-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'JN-IA OS'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
> "Pronto. O JN-IA OS já te conhece.
>
